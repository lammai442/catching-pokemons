...
const soundBtnRef = document.querySelector('#sound-btn');
const victorySoundBtnRef = document.querySelector('#victory-sound-btn');
...
function validateForm() {
	...
	try {
		...
		if (nickRef.value.length < 5 || nickRef.value.length > 10) {
			nickRef.focus();
			throw new Error(
				'Your name may only be between 5 and 10 characters'
			);
...
function endGame() {
	...
	oGameData.nmbrOfSeconds =
		Math.round(oGameData.nmbrOfMilliseconds() / 10) / 100;
...
function viewHighScore(highScore) {
	...
	document.querySelector(
		'#winMsg'
	).textContent = `Well done ${oGameData.trainerName}! Your time was ${oGameData.nmbrOfSeconds} seconds.`;
